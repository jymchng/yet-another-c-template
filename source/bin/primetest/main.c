...
// Converts a string argument arg to an unsigned long long value referenced by
// val. Returns true if the argument conversion succeeds, and false if it fails.
static bool convert_arg(const char* arg, unsigned long long* val)
{
  ...
  *val = strtoull(arg, &end, 10);
  ...
  if (end == arg)
    return false;
  // If we got here, we were able to convert the argument. However, we want to
  // allow only values greater than one, so we reject values <= 1.
  if (*val <= 1)
    return false;
  return true;
}
